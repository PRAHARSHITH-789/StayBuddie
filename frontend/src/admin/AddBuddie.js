...
useEffect(() => {
  // Function to fetch the notice period status for each buddy
  const fetchNoticeStatus = async () => {
    try {
      // Loop through each buddy and fetch their notice period status
      const statusPromises = buddies.map(async (buddie) => {
        const response = await axios.get(`${process.env.REACT_APP_URL}/admin/buddie/notice-status/${buddie._id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}` },
        });
        return { id: buddie._id, status: response.data.isServingNotice };
      });

      // Wait for all the status to be fetched
      const statuses = await Promise.all(statusPromises);
      
      // Map the results into an object for easier access
      const statusMap = statuses.reduce((acc, curr) => {
        acc[curr.id] = curr.status;
        return acc;
      }, {});

      // Update the state with the fetched notice statuses
      setNoticeStatus(statusMap);
    } catch (error) {
  ...
  fetchNoticeStatus();
}, [buddies]);  // Only re-fetch when 'buddies' list changes
    ...
    if (isValid) {
      try {
       ...
       setFormData({
        ...
        toast.error('Filed to add buddie already exists');
      ...
      >
        <Box display="flex" alignItems="center" gap={2}>
          ...
          <Box display="flex" flexDirection="column">
            ...
            {/* Display 'In Notice Period' if the Buddie is serving notice */}
            {noticeStatus[buddie._id] && (
              <Chip label="In Notice Period" color="secondary" size="small" sx={{ marginTop: 1 }} />
            )}
          </Box>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
