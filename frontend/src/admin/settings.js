  ...
  useEffect(() => {
    socket?.on("complaintRequest", (data) => {
      if(pendingComplaintsCount > 0){
        setPendingComplaintsCount(prevComps => prevComps + 1)
       
      }
      else{
        setPendingComplaintsCount(1)
      }
    });
  }, [socket, pendingComplaintsCount]);
  ...
  useEffect(() => {
    socket?.on("paymentRequest", (data) => {
      if(pendingPaymentsCount > 0){
        setPendingPaymentsCount(prevPay => prevPay + 1);
      }else{
        setPendingPaymentsCount(1);
      }
      
    });
  }, [socket,pendingPaymentsCount]);
    ...
    <Container maxWidth="sm" sx={{ mt: 4 }}>
      ...
      <Box sx={{ bgcolor: "background.paper", borderRadius: 2, boxShadow: 3 }}>
        <List sx={{ py: 0 }}>
          ...
          <ListItem button sx={{ py: 2 }} component={Link} to="/admin/notice-period">
            <ListItemIcon>
              <ExitToAppSharp />
            </ListItemIcon>
            <ListItemText primary="Notice Period" />
          </ListItem>
          <Divider />
